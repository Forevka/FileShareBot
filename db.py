...
async def insert_user(user_id, user_name, user_lang):
    conn = await asyncpg.connect(user='postgres', password='werdwerd',
                                 database='files_new', host='194.67.205.185')
    values = await conn.execute("""INSERT INTO users(user_id, user_name, user_lang) VALUES('{}', '{}', '{}') ON CONFLICT (user_id) DO NOTHING RETURNING 0;""".format(str(user_id).replace("'","''"), user_name, user_lang))
    await conn.close()
    if values.split(" ")[-1]=='0':
        return 0; #nothing added
    else:
